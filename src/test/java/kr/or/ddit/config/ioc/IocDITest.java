...
public class IocDITest {
	...
	// 컨테이너를 만들으려면 설정 파일이 필요 (ContextConfiguration()) -> (locations = {위치}, value)를 사용
	
	// DI
	// 스프링 빈중에 호횐되는 타입의 빈이 있을 때 주입
	@Autowired
	ApplicationContext context; // 동일한 파일이 있는지  
	...
	// boardService, boardServiceC 스프링빈 주입한 barodRepository 스트링빈은 
	// 동일한 빈이므로 두 객체의 getter 메소드를 통해 얻어온 boardRepository 객체를 동일해야 한다.
	
	
	
	@Test
	public void DIAutowiredText() {
		/***Given***/
