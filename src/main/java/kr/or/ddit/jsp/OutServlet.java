...
public class OutServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * ranger 정보를 화면에 표현해줄 out.jsp 파일로 응답 생성을 위임 ranger 정보가 어딘가 담겨야 함 servlet 에서는
		 * 3가지 scope가 사용 가능 request < session < application 해당요청은 일회성으로만 처리해주면 레인저 이름
		 * 정보를 보관할 필요가 없고 다른 요청과 관련이 없으므로 request 적당하다
		 */
		...
		/*
		 * List<String> rangers = new ArrayList<String>(); rangers =
		 * rangerService.getRanger();
		 */
